...
:END:
 ...
 |----------------+---------+-------------------------------|
 | FOSSIL_VERSION | 2.18    | Set upload fossil version     |
 |----------------+---------+-------------------------------|
...
#+begin_example
  docker build \
         ...
         --build-arg VERSION='22.06' \
         --build-arg FOSSIL_VERSION='2.18' \
         -t oupfiz5/tcl-build:22.05 \
         -f ./Dockerfile \
          .
#+end_example
...
|--------------------+---------+-------------------------------------|
| RL_JSON_VERSION    |  0.11.1 | Define [[https://github.com/RubyLane/rl_json][RL_JSON]] version              |
|--------------------+---------+-------------------------------------|
...
#+begin_src sh :shebang ""
  # * Versions
  FOSSIL_VERSION="${FOSSIL_VERSION:-2.18}" \
  ...
  CONTAINER_NAME="${CONTAINER_NAME:-tcl-build}"
  IMAGE_REPOSITORY="${IMAGE_REPOSITORY:-oupfiz5}"
  IMAGE_NAME="${IMAGE_NAME:-tcl-build}"
  IMAGE_TAG="${IMAGE_TAG:-22.08}"
  UBUNTU_IMAGE_REPOSITORY="${UBUNTU_IMAGE_REPOSITORY:-oupfiz5}"
  UBUNTU_IMAGE_NAME="${UBUNTU_IMAGE_NAME:-ubuntu-s6}"
  UBUNTU_IMAGE_TAG="${UBUNTU_IMAGE_TAG:-22.04-3.1.2.1}"
#+end_src
  ...
  jobs:
    checks:
      ...
      steps:
        ...
        - name: Tcl-build. Build. Check image and container. Check builds.
          run: |
...
#+begin_src sh :shebang ""
  ...
  # * Add
  ADD https://www.fossil-scm.org/home/uv/fossil-linux-x64-2.18.tar.gz /tmp/fossil-linux.tar.gz
...
** builds/tcl-thread-build.sh :code:
:PROPERTIES:
:CUSTOM_ID: builds-tcl-thread-build-sh
:END:
#+begin_src sh
  # shellcheck source=../src/builds/common.sh
  . /builds/common.sh
  ...
  # cut down on the output to stdout to make Travis-CI consoles faster
  make install 2>&1 | tee -a /workspaces/logs/"${log_nam}" | cut -c1-64
